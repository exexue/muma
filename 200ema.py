...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        for i, d in enumerate(self.datas):
            ...
            # 100日移动平均线
            self.inds[d]['ema200'] = bt.indicators.ExponentialMovingAverage(d,period=200)
            # 100 日真实波动ATR
            self.inds[d]['atr100'] = bt.indicators.AverageTrueRange(d, period=100)
            # MACD指标
            self.inds[d]['macd'] = bt.indicators.MACD(d)
            #MACD买入信号判断
            self.inds[d]['buy_signal'] = bt.indicators.CrossOver(self.inds[d]['macd'].macd,self.inds[d]['macd'].signal) 
    ...
    def next(self):
        ...
        for i, d in enumerate(self.datas):
            ...
            if not pos:  # no market / no orders
                if self.inds[d]['buy_signal'][0] == 1 and d.close[0] > self.inds[d]['ema200'][0]:
                    
                        self.order = self.buy(data=d,size=int(1000000/d.close[0]))
                ...
                if self.inds[d]['buy_signal'][0] == -1 and d.close[0] < self.inds[d]['ema200'][0]:
                    self.order = self.sell(data=d,size=int(1000000/d.close[0]))
                ...
                if self.getposition(d).size > 0  and d.close[0] < self.getposition(d).price - 2*self.inds[d]['atr100'][0]:
                        print(self.getposition(d).price)
                        self.order = self.close(data=d)
                        self.log('平多止损品种:' + d._name + ',平多价格： %.2f' % d.close[0])
                        if dt_str == now_str:
                            print("当前时间发出平多信号,正在推送TG...")
                            #bot.send_message(chat_id='727256696', text="品种:%s ,价格:%.2f ,发出平多信号！" % (d._name,d.close[0]))
                if self.getposition(d).size > 0 and  d.close[0] > self.getposition(d).price + 3*self.inds[d]['atr100'][0]:
                        self.order = self.close(data=d)
                        self.log('平多止盈品种:' + d._name + ',平多价格： %.2f' % d.close[0])
                        
                        

                if self.getposition(d).size < 0 and d.close[0] > self.getposition(d).price + 2*self.inds[d]['atr100'][0]:
                        self.order = self.close(data=d)
                        self.log('平空止损品种:' + d._name + ',平空价格： %.2f' % d.close[0])
                        if dt_str == now_str:
                ...
                if self.getposition(d).size < 0 and  d.close[0] < self.getposition(d).price - 3*self.inds[d]['atr100'][0]:
                        print(self.getposition(d).price)
                        self.order = self.close(data=d)
                        self.log('平空止盈品种:' + d._name + ',平空价格： %.2f' % d.close[0])
...
if __name__ == '__main__':
    ...
    code_range =['C0','M0','RM0','JD0','TA0','MA0','FG0','PF0','SA0','UR0','BU0','FU0',
                 'CU0','AL0','ZN0','AU0','AG0','RB0','RU0','HC0','SP0','SS0','P0','J0','Y0','JM0','I0','AP0','CJ0',
                 'SR0','CF0']
    ...
    for code_name in code_range:
        dataname = get_data(code_name,"30")
        print(dataname)
        data = bt.feeds.PandasData(dataname=dataname,timeframe=bt.TimeFrame.Minutes,compression=30)  # 加载数据
        cerebro.adddata(data,name = code_name)  # 将数据传入回测系统
    
    # 设定初始资金和佣金
    cerebro.broker.setcash(23000000)
    ...
    cerebro.addanalyzer(bt.analyzers.TradeAnalyzer, _name='mun')
    




    print(f'资金开始Starte Portfolio Value {cerebro.broker.getvalue()}')
    result = cerebro.run()

    print('----------------------------')
    print(f'资金结果End portfolio value {cerebro.broker.getvalue()}')
    print('----------------------------')
    print(f"资金结果百分比Total Return:  {round(result[0].analyzers.returns.get_analysis()['rtot']*100, 2)}%")
    print(f"APR:           {round(result[0].analyzers.returns.get_analysis()['rnorm100'],2)}%")
    print(f"最大回测Max DrawDown:  {round(result[0].analyzers.drawdown.get_analysis()['max']['drawdown'],2)}%")
    print(f"夏普比率Sharpe Ratio:  {round(result[0].analyzers.sharpe.get_analysis()['sharperatio'],2)}")
    #print(result[0].analyzers.mun.get_analysis())
