...
#%matplotlib auto
# import telegram
# TOKEN = '1454306275:AAHbxYSgcGBoIY3t5k0B'
        ...
        for i, d in enumerate(self.datas):
            ...
            if not pos:  # no market / no orders
                if self.inds[d]['sma50'][0] > self.inds[d]['sma100'][0] and d.close[0] > self.inds[d]['High50'][0]:
                        cash=(0.0025*10000000)/(3*self.inds[d]['atr100'][0]/d.close[0])
                        print(cash)
                        size = cash/d.close[0]
                        print(size)
                        ...
                        if dt_str == now_str:
                            ...
                            #bot.send_message(chat_id='7272566', text="品种:%s ,价格:%.2f ,发出做多信号！" % (d._name,d.close[0]) )
                        

                if self.inds[d]['sma50'][0]  < self.inds[d]['sma100'][0] and d.close[0] < self.inds[d]['Low50'][0]:
                    cash=(0.002*10000000)/(3*self.inds[d]['atr100'][0]/d.close[0])
                    print(cash)
                    size = cash/d.close[0]
                    print(size)
                ...
                if self.getposition(d).size > 0  and d.close[0] < self.inds[d]['High50'][0] - 3*self.inds[d]['atr100'][0]:
                        ...
                        if dt_str == now_str:
                            ...
                            #bot.send_message(chat_id='72725', text="品种:%s ,价格:%.2f ,发出平多信号！" % (d._name,d.close[0]))
                ...
                if self.getposition(d).size < 0 and d.close[0] > self.inds[d]['Low50'][0] + 3*self.inds[d]['atr100'][0]:
                        ...
                        if dt_str == now_str:
                            ...
                            #bot.send_message(chat_id='7272566', text="品种:%s ,价格:%.2f ,发出平空信号！" % (d._name,d.close[0]))
