...
def get_data(code,time):
            ...
            data=data.astype({
                    ...
                    'high':'float',
                    'low':'float',
                    'close':'float',#期货多品种60分钟日ATR0.02波动系统（下一个交易日开盘买入实现）
import datetime  # For datetime objects
...
def Tg_send(time,code,signal,close,size):
            ...
            if time == now_str:
                ...
                if x in f :
                    ...
                    bot.send_message(chat_id='727256696', text="品种:%s ,价格:%.2f ,发出%s信号！" % (code,close,signal))
                    if signal == "做多":
                        autotrade.buy_open(code[:-4],sizes)
                        tm.sleep(2)
                    if signal == "做空":
                        autotrade.sell_open(code[:-4],sizes)
                        tm.sleep(2)
                    if signal == "平多":
                        autotrade.sell_close(code[:-4],sizes)
                        tm.sleep(2)
                    if signal == "平空":
                        autotrade.buy_close(code[:-4],sizes)
                        tm.sleep(2)
                    os.system('echo %s,code=%s,%s >>/code/log.txt' % (time,code,signal) )
class TestStrategy(bt.Strategy):
    def log(self, txt, dt=None):
