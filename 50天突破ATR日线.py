...
def get_data(code,time):
            data=ak.futures_main_sina(symbol=code, trade_date="20181220")
            data.columns = ['date','open','high','low','close','volume','hold']
...
#                  'SR0','CF0']
    ...
    start_date = datetime.datetime(2005, 6, 1)  # 回测开始时间
    ...
    for code_name in code_range:
        ...
        print(dataname)
        data = bt.feeds.PandasData(dataname=dataname,fromdate=start_date,todate=end_date,timeframe=bt.TimeFrame.Days,compression=1)  # 加载数据
    ...
    #print(result[0].analyzers.tarde.get_analysis())
