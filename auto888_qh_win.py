...
def get_data(code,time):
            ...
            tm.sleep(1)
            return data 
def Tg_send(time,code,signal,close,size,position):
            now=datetime.datetime.now()
            now_str=now.strftime('%Y-%m-%d')
            #now_str = "2021-05-21"
            if time == now_str:
                sizes = autotrade._size(code,size) #计算实盘用的交易手数
                f = open('log_qh.txt', 'r').read()
                x = "%s,code=%s,%s" % (time,code,signal)
                if x in f :
                    print("当天已经执行过了")
                else:
                    print("发出信号，正在执行实盘...")
                    os.system("python c:/autotrade.py")  # 登录交易软件
                    pos=100;close_pos = 100
                    name = get_dominant_future(code[:-4],now_str)
                    print(code)
                    if signal == "做多":
                        pos = autotrade.buy_open(name[:-5],sizes)    #如果返回1说明软件中有仓位。
                        #print(code[:-4])
                        tm.sleep(2)
                    if signal == "做空":
                        pos = autotrade.sell_open(name[:-5],sizes) #如果返回1说明软件中有仓位
                        #print(pos)
                        tm.sleep(2)
                    if signal == "平多":                                  
                        close_pos= autotrade.sell_close(name[:-5],sizes)   #如果返回0说明软件中无仓位。
                        tm.sleep(2)
                    if signal == "平空":
                        close_pos= autotrade.buy_close(name[:-5],sizes)   #如果返回0说明软件中无仓位。
                        tm.sleep(2)
                    if pos == 1 or close_pos ==0:   #如果执行命令成功
                        print("实盘执行成功，当前时间发出%s信号,正在推送TG..."% (signal))
                        bot.send_message(chat_id='727256696', text="品种:%s ,价格:%.2f ,数量:%s,发出%s信号,实盘执行成功！当前持仓：%s" % (code,close,sizes,signal,position))
                        os.system('echo %s,code=%s,%s  >>log_qh.txt' % (time,code,signal) )

                    else:
                        print("实盘执行失败，请检查软件运行情况。")
                        bot.send_message(chat_id='727256696', text="品种:%s ,价格:%.2f ,数量:%s,发出%s信号，但是实盘执行失败，请检查软件运行情况！当前持仓：%s" % (code,close,sizes,signal,position))
    ...
    def notify_order(self, order):
        ...
        if order.status == order.Completed:
            ...
            if order.isbuy():
                ...
                Tg_send(time=index_date, code = order.p.data._name, signal = signal, close = order.executed.price, size=size,position = str(self.position_dict.keys()))
...
if __name__ == '__main__':
    ...
    quantstats.reports.html(returns,output='./data/index666.html', title=f' 期货多品种交易系统60m ATR0.001 Analysis')
